...
class CdcatDatabase: # {{{
    """Class which implements functions to parse a cdcat database and generate copy scripts based on it."""
    ...
    def __init__(self, directory_separator = os.path.sep, shell = 'sh'):
        ...
        # /s: Copies subdirectories. Note that this option excludes empty directories. (Empty directories are not consider as useful).
    ...
    def parse_gzip_file(self, gzip_file, generate_formats=[ 'git-annex' ]):
        """Parse cdcat database and generate the copy script using the specified programs (in generate_formats).

        :param gzip_file: path to the gzipped XML file from cdcat.
        :param generate_formats: list with the program which should be used in the copy script (default is git-annex).
        """
    ...
    def _for_catalog(self, catalog):
        logging.info(u'Parsing catalog called "%s" owned by "%s" …' % (catalog.attrib['name'], catalog.attrib['owner']))
        count = 1
    ...
    def _for_media(self, media):
        ...
        if 'cmd' == self._executing_shell:
            print 'set %s=d:\\external_disk\\' % self.__name_of_target_path_variable
        ...
        print '\n%s Do not change the following commands. They can be regenerated be the script after altering the database with cdcat (in case you want to select other files as listed here).' % (
                self._start_line_comment_token
                )
        ...
        if 'robocopy' in self._generate_formats:
            ...
            if len(unwanted) != 0:
                logging.warning('robocopy does not support to exclude files or directories.'
                        + ' Directories(s) or file(s) "%s" will be copied although they where excluded.' % ' '.join(unwanted))

            print 'robocopy "%s%s%s" "%s\\%s" %s' % (
                    self._get_expandable_variable(self.__name_of_base_path_variable), self._directory_separator, node_path,
                    self._get_expandable_variable(self.__name_of_target_path_variable),
                    cur_path,
