...
class CdcatDatabase:  # {{{
    ...
    def parse_gzip_file(self, gzip_file, generate_format='git-annex'):
        """
    ...
    def parse_uncompressed_xml_file(
    ...
    def _for_media(self, media):
        ...
        if 'sh' == self._executing_shell:
            print '#!/bin/sh'
        print '{} This OS dependent copy script was generated by {}.'.format(
            self._start_line_comment_token,
            SCRIPT_URL
        )
        print '{} You will need to change the base path to the media "{}"'.format(
            self._start_line_comment_token,
            media.attrib['name']
            ) \
            + ' and the target path.\n'
        if 'git-annex' == self._generate_format:
            print '{} Path to the base directory of the media'.format(
                self._start_line_comment_token,
                ) \
        ...
        if 'cmd' == self._executing_shell:
            print 'set {}=d:\\external_disk\\'.format(
                self.__name_of_target_path_variable
            )
        if 'git-annex' not in self._generate_format:
            print '{} The copy script will create one directory' \
                + ' for each media under "{1}".'.format(
                    self._start_line_comment_token,
    ...
    def _add_to_copy_list(
        ...
        if 'git-annex' in self._generate_format:
            ...
            # print 'popd'
        if 'robocopy' in self._generate_format:
